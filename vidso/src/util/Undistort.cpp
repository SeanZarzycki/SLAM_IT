...
ImageAndExposure* Undistort::undistort(const MinimalImageB* image_raw, const MinimalImageB3* col_raw, float exposure, double timestamp, float factor) const
{
	if(image_raw->w != wOrg || image_raw->h != hOrg)
	{
		printf("Undistort::undistort: wrong image size (%d %d instead of %d %d) \n", image_raw->w, image_raw->h, w, h);
	...
	photometricUndist->processFrame<unsigned char>(image_raw->data, exposure, factor);
	ImageAndExposure* result = new ImageAndExposure(w, h, timestamp);
	photometricUndist->output->copyMetaTo(*result);
	...
	if (!passthrough)
	{
		float* out_r = result->ri;
		float* out_g = result->gi;
		float* out_b = result->bi;
		float* out_data = result->image;
		float* in_data = photometricUndist->output->image;
	...
	{
		memcpy(result->image, photometricUndist->output->image, sizeof(float)*w*h);
		memcpy(result->ri, in_r, sizeof(float)*w*h);
		memcpy(result->gi, in_g, sizeof(float)*w*h);
		memcpy(result->bi, in_g, sizeof(float)*w*h);
	}
