...
PointHessian::PointHessian(const ImmaturePoint* const rawPoint, CalibHessian* Hcalib)
{
	...
	memcpy(rc, rawPoint->rc, sizeof(float)*n);
	memcpy(gc, rawPoint->gc, sizeof(float)*n);
	memcpy(bc, rawPoint->bc, sizeof(float)*n);
...
void FrameHessian::makeImages(float* color, float* ri, float* gi, float* bi, CalibHessian* HCalib)
{
	...
	dr = new Eigen::Vector3f[wG[0]*hG[0]];
	dg = new Eigen::Vector3f[wG[0]*hG[0]];
	db = new Eigen::Vector3f[wG[0]*hG[0]];
	...
	{
		float dx = 0.5f*(dr[idx+1][0] - dr[idx-1][0]);
		float dy = 0.5f*(dr[idx+w][0] - dr[idx-w][0]);
		if(!std::isfinite(dx)) dx=0;
		if(!std::isfinite(dy)) dy=0;
		dr[idx][1] = dx;
		dr[idx][2] = dy;

		dx = 0.5f*(dg[idx+1][0] - dg[idx-1][0]);
		dy = 0.5f*(dg[idx+w][0] - dg[idx-w][0]);
		if(!std::isfinite(dx)) dx=0;
		if(!std::isfinite(dy)) dy=0;
		dg[idx][1] = dx;
		dg[idx][2] = dy;

		dx = 0.5f*(db[idx+1][0] - db[idx-1][0]);
		dy = 0.5f*(db[idx+w][0] - db[idx-w][0]);
		if(!std::isfinite(dx)) dx=0;
		if(!std::isfinite(dy)) dy=0;
		db[idx][1] = dx;
		db[idx][2] = dy;
