...
: u(u_), v(v_), host(host_), my_type(type), idepth_min(0), idepth_max(NAN), lastTraceStatus(IPS_UNINITIALIZED)
{
	...
	for(int idx=0;idx<patternNum;idx++)
	{
		...
		Vec3f ptc_r = getInterpolatedElement33BiLin(host->dr, u+dx, v+dy,wG[0]);
		Vec3f ptc_g = getInterpolatedElement33BiLin(host->dg, u+dx, v+dy,wG[0]);
		Vec3f ptc_b = getInterpolatedElement33BiLin(host->db, u+dx, v+dy,wG[0]);
